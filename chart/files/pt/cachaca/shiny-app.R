...
# Configuration
METRICS_URL <- Sys.getenv("METRICS_URL", "http://assistente-cachaca:8080/metrics")
...
parse_prometheus_metrics <- function(metrics_text) {
  ...
  # Calculate approved requests (input + output detections from direction metrics)
  metrics$approved_requests <- max(0, metrics$total_requests - metrics$input_blocked - metrics$output_blocked)
...
ui <- page_fillable(
  ...
  titlePanel("🥃 Dashboard de guardrails - Cachaça"),

  div(class = "subtitle", "Monitoramento em tempo real das detecções e métricas do assistente de cachaça (Brasil)"),

  layout_columns(
    col_widths = c(3, 3, 3, 3),

    div(class = "metric-card",
        div(class = "metric-label", "Total de consultas"),
        div(class = "metric-value total", textOutput("total_requests"))
    ),

    div(class = "metric-card",
        div(class = "metric-label", "Entrada bloqueada"),
        div(class = "metric-value blocked-input", textOutput("input_blocked"))
    ),

    div(class = "metric-card",
        div(class = "metric-label", "Respostas bloqueadas"),
        div(class = "metric-value blocked-output", textOutput("output_blocked"))
    ),

    div(class = "metric-card",
        div(class = "metric-label", "Consultas aprovadas"),
        div(class = "metric-value approved", textOutput("approved_requests"))
    )
  ),

  h2("Detecções por guardrail"),
  uiOutput("detector_bars"),

  div(class = "last-updated", textOutput("last_updated")),

  div(class = "footer", "Feito pela CAI Team • Powered by OpenShift AI")
)
...
server <- function(input, output, session) {
  ...
  output$detector_bars <- renderUI({
    ...
    if (is.null(m) || length(m$detections_by_detector) == 0) {
      return(div("Sem detecções ainda"))
    }
    ...
    bars <- lapply(names(detectors_sorted), function(detector) {
      ...
      display_name <- switch(detector,
                             "hap" = "🤬 Linguagem inadequada",
                             "language" = "🇧🇷 Não português",
                             "language_detection" = "🇧🇷 Não português",
                             "prompt_injection" = "👮 Jailbreak",
                             "regex_fruit" = "🥃 Não cachaça",
                             "regex_competitor" = "🥃 Não cachaça",
                             "pii" = "🔒 PII",
                             "message_length" = "📏 Comprimento",
                             "topic_relevance" = "🍑 Fora do tema",
                             detector)
  ...
  output$last_updated <- renderText({
    m <- metrics_data()
    if (is.null(m)) return("")
    paste("Última atualização:", format(Sys.time(), "%H:%M:%S"))
  })
}
