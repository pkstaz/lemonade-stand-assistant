...
# Configuration
METRICS_URL <- Sys.getenv("METRICS_URL", "http://assistente-cafe:8080/metrics")
...
ui <- page_fillable(
  ...
  titlePanel("☕ Dashboard de guardrails - Café"),

  div(class = "subtitle", "Monitoramento em tempo real das detecções e métricas do assistente de café"),
  ...
  output$detector_bars <- renderUI({
    ...
    bars <- lapply(names(detectors_sorted), function(detector) {
      ...
      display_name <- switch(detector,
                             ...
                             "regex_fruit" = "☕ Não café",
                             "regex_competitor" = "☕ Não café",
                             "pii" = "🔒 PII",
