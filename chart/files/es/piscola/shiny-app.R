...
# Configuration
METRICS_URL <- Sys.getenv("METRICS_URL", "http://asistente-piscola:8080/metrics")
...
ui <- page_fillable(
  ...
  titlePanel("🥃 Dashboard de guardrails - Piscola"),

  div(class = "subtitle", "Monitoreo en tiempo real de detecciones y métricas del asistente de piscola (Chile)"),
...
server <- function(input, output, session) {
  ...
  output$detector_bars <- renderUI({
    ...
    if (is.null(m) || length(m$detections_by_detector) == 0) {
      return(div("Sin detecciones aún"))
    }
    ...
    bars <- lapply(names(detectors_sorted), function(detector) {
      ...
      display_name <- switch(detector,
                             "hap" = "🤬 Lenguaje inapropiado",
                             "language" = "🇪🇸 No español",
                             "language_detection" = "🇪🇸 No español",
                             "prompt_injection" = "👮 Jailbreak",
                             "regex_fruit" = "🥃 No piscola",
                             "regex_competitor" = "🥃 No piscola",
                             "pii" = "🔒 PII",
                             "message_length" = "📏 Longitud",
                             "topic_relevance" = "🍑 Fuera de tema",
                             detector)
