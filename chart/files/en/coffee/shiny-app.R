...
# Configuration
METRICS_URL <- Sys.getenv("METRICS_URL", "http://coffee-assistant:8080/metrics")
...
ui <- page_fillable(
  ...
  titlePanel("☕ Coffee guardrails dashboard"),
  ...
  output$detector_bars <- renderUI({
    ...
    bars <- lapply(names(detectors_sorted), function(detector) {
      ...
      display_name <- switch(detector,
                             ...
                             "regex_fruit" = "☕ Non Coffee",
                             "regex_competitor" = "☕ Non Coffee",
                             "pii" = "🔒 PII",
